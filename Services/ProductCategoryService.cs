...
{
    private readonly IEntityStore<ProductCategory> _store;
    private readonly ILogger<ServiceBase<ProductCategory>> _logger;
    
    public ProductCategoryService(ILogger<ServiceBase<ProductCategory>> logger, IEntityStore<ProductCategory> store) : base(logger, store)
    {
        _logger = logger;
        _store = store;
    }
    ...
    public async Task<IReturnObject<ProductCategory>> CreateCategory(string newCategory)
    {
        try
        {
            var categoryExist = await _store.Exists(c => c.Name.ToLower() == newCategory.ToLower());
            if (categoryExist) return new ReturnObject<ProductCategory>().BadRequest("Category already exists");

            var category = await _store.Create(new ProductCategory() { Name = newCategory });
